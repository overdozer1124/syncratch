...
5b. **同一スプライト・異なる stack**
   ...
   - forever 入れ子と別 stack 追加、detach と別 block の field 変更が同時でも双方残る（自動試験で固定）
6. **選択維持（B）**
...
実 Chromium 2-context 試験には、上記の Basketball・別ターゲット収束・**選択維持**・**local UI（tab/viewport）**・招待 origin 一致が含まれる。同一スプライト異なる stack は `e2e/collab.spec.ts` の **2 実 Chromium context + collaboration harness**（WebRTC 実搬送）で固定し、Scratch GUI / Blockly 表示収束そのものは別試験とする。
...
- 同一 target に per-block `blocks` map と legacy `blocksJson` が同時に存在する場合は fail-closed。暗黙の二重 writer にしない

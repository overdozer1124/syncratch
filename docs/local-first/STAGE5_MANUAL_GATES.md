...
| 文書更新基準 tip（main） | `eb045b9ce6d70bbbc16f023b176738813196636d`（2026-07-29） |
...
### C.1.1 再プローブ（2026-07-29 Cursor）

```text
実施日 (JST): 2026-07-29 12:04:32 JST
実施者: Cursor（Stage 5 支援・文書更新）
git tip（main / 文書ブランチ基準）: eb045b9ce6d70bbbc16f023b176738813196636d
作業ブランチ: cursor/release-gates-stage5-23c9（旧 Draft #21 を現行 main へ載せ替え）
検証 origin: https://syncratch-production.up.railway.app/

本番再確認:
- GET /healthz → ok (HTTP 200)
- GET / → HTTP 200
- production /assets/main-CZ1vWpJg.js:
  - Client ID 863099193805-….apps.googleusercontent.com あり
  - APP_ID 863099193805 あり
  - scope drive.file = 1 / drive.readonly = 0
  - 「あなたはホスト（リンクを作った人）」出現 1 回（近傍二重なし）
  - 切断文言「友だちとのつながりが切れました」あり
  - ゲスト Drive ブロック文言あり

focused unit（editor-web）:
- status-icons / collab-role-ui / project-status / ui-copy: 21/21 PASS
  （online ホバーのホスト文言が label 内で 1 回だけになることを含む）

手動ゲート進捗（変更なし）:
A1–A4 / B2: PASS（2026-07-23 記録を維持）
A5 / A6 / A7 / B1 / B3: 未実施（ユーザー実機）

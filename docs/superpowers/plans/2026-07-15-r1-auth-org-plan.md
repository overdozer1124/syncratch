...
- Modify: `packages/project-store-sqlite/src/migrate.ts` — call `migrateAuth(db)` after project tables; keep `PRAGMA foreign_keys=ON`
...
- Modify: `packages/project-store-sqlite/src/repository.ts` / `index.ts` — export `openSqliteDatabase(path)` **or** `openSqliteProjectRepository` returns `{ db, projectRepo, authRepo, close }` so bootstrap holds **one** close

**Pinned connect lifecycle:**
1. `bootstrap` → `const store = openSqliteStore(dbPath)` // opens once

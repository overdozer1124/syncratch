# Release 1 Slice 窶・Google Auth, Session & Organization Foundation Design

> **Date:** 2026-07-15
> **Status:** Revised draft for re-review (do not implement until approved)
> **Revision:** CSRF cookie supply + Origin rules; immutable `(sub竊弛rg)` bind (post-`efc4a24`)
> **Baselines:** Gate 0 Technical Go @ `4a14e05`; R1 persistence Technical Go @ `3d6053b`
> **Spec anchors:** ﾂｧ6窶・, ﾂｧ44窶・5, ﾂｧ46, ﾂｧ55
...
**Final Go** requires fixture suite green **and** real Google GIS + allowed Workspace `hd` evidence. Without credentials: **Technical Go 窶・real GIS conditional**.
...
```text
Gate 0:  verifyGoogleIdToken  笏笏笏・                                笏・unconnected
R1:      StubAuthContext(x-user-id) 竊・ProjectService 竊・Durable ACL (SQLite)
```

`GoogleIdentityClaims` today has **no `name`**. This slice does **not** extend that type for display-name sync (see ﾂｧ9.5).
...
| B. App JWT session | Stateless cookie JWT | Rejected 窶・revoke/logout weaker |
| C. Redis session service | A + external store | Deferred 窶・ops weight |

## 4. Architecture

```text
POST /v1/auth/google   (no CSRF; strict Origin + JSON CT + CORS)
   竊・verifyGoogleIdToken(+ authorizedParties when configured)
   竊・single sync TX: org/user/identity/membership/session (first login)
   竊・Set-Cookie: blocksync_session (HttpOnly) + blocksync_csrf (NOT HttpOnly)
   笆ｼ
Mutating APIs (logout, project write, snapshot, restore)
   竊・Origin required + allow-listed
   竊・Session cookie + X-CSRF-Token (or csrf cookie value) matches sessions.csrf_hash
   笆ｼ
SessionAuthContext.resolve
   竊・session hash OK + not expired/revoked
   竊・user.status + org.status active
   竊・membership(organization_id, user_id) EXISTS (else 401)
   竊・AuthPrincipal
```
...
- **Migration order:** existing project migrate 竊・auth migrate (`migrateAuth(db)`) in the same boot path
...
|---|---|---|
| `R1_AUTH_MODE` | **Must be `google`** 窶・refuse stub | `stub` or `google` |
...
1. **First successful login** permanently binds `(provider='google', subject=sub)` to exactly one `organization_id` (the org resolved from that login窶冱 verified `hd`). Store this on `external_identities.organization_id` and set `users.primary_organization_id` once.
2. **Re-login:** after token verify, if `claims.hd` resolves to a **different** org than `external_identities.organization_id` 竊・**generic `AUTH_FAILED`**. Do **not** update `primary_organization_id`, do **not** add memberships.
...
5. **Session resolve (every request):** require `users.status=active`, `organizations.status=active`, and a current `organization_memberships` row for `(session.organization_id, session.user_id)`. If membership was removed 竊・**401** even if session Cookie still valid / unexpired.
...
- Missing / mismatched / not allow-listed hd 竊・`AUTH_FAILED`.
...
| Value | `randomBytes(32).toString("base64url")` (竕･256-bit) |
| HttpOnly | `true` |
| Secure | guarded (ﾂｧ5) |
| SameSite | `Lax` |
| Path | `/` |
...
`/v1/auth/me` returns user profile + `expiresAt` but **does not** need to reconstruct CSRF from the hash 窶・the browser already has `blocksync_csrf`. Me may omit `csrfToken` JSON (cookie is source of truth after reload).
...
|---|---|---|---|
| `POST /v1/auth/google` | no | **no** | **Required**; must be in `R1_ALLOWED_ORIGINS`. Missing / `null` / disallowed 竊・**403** |
| `GET /v1/auth/me`, `GET` project reads | yes | no | n/a (safe) |
| `POST /v1/auth/logout` | yes | **yes** | **Required** allow-list; missing/null/disallowed 竊・**403** |
| Project mutations (`POST/PUT` create/save/snapshot/restore) | yes | **yes** | **Required** allow-list; missing/null/disallowed 竊・**403** |
...
**Do not** use 窶忻alidate Origin only if present窶・for browser mutating APIs or login.
...
Expired / `revoked_at` set / membership missing / user or org inactive 竊・401. Logout revokes + clears both cookies.
...
Failure: `401` `{ "code": "AUTH_FAILED" }` or Origin/CT failures `403`/`400` as appropriate 窶・never leak `VerifyFailureCode`.
...
| Env | Behavior |
|---|---|
| `R1_GOOGLE_AUTHORIZED_PARTIES` unset / empty | Pass **no** `authorizedParties` to verifier (azp **not** enforced) 窶・acceptable for simple GIS ID-token-only web client where `aud` alone matches Client ID |
...
| Cross-tenant via hd change | Immutable sub竊弛rg bind; mismatch 竊・AUTH_FAILED |
...
- `/v1/auth/google` without Origin 竊・403; wrong Origin 竊・403; with CSRF header alone still works without prior CSRF (login has no CSRF)
- Mutation without Origin 竊・403 even with valid session+csrf
- Same `sub` + different allow-listed `hd` on re-login 竊・AUTH_FAILED; user/org unchanged
- Membership deleted 竊・existing session 竊・401
- Concurrent dual first-login 竊・one user/identity
...
- [ ] Immutable sub竊弛rg bind + membership-on-resolve
- [ ] Single-connection ownership + migrate order
- [ ] FK/CHECK/PRAGMA; session竊知embership composite FK

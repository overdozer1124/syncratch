...
> **Revision:** UTF-8 restore + acyclic packages + CSRF both-required + CORS preflight (post-`e3fb88a`)
...
Mutating APIs (logout, project write, snapshot, restore)
   ...
   → CSRF Cookie + X-CSRF-Token **both** required; equal; sha256 matches sessions.csrf_hash
   ▼
...
```text
auth-context          AuthContext / AuthPrincipal / StubAuthContext only
       ▲
session-service       AuthRepository port + SessionAuthContext + login/logout/me
       ▲
project-store-sqlite  implements AuthRepository (+ ProjectRepository); opens DB
       ▲
r1-persist-server     wires cookies / CSRF / Origin / bootstrap
```

- `auth-context` **must not** import `project-store-sqlite` or `session-service`.
- `SessionAuthContext` lives in `@blocksync/session-service` and depends on the `AuthRepository` **port** defined there.
- `project-store-sqlite` depends on `session-service` (for the port type) and `project-service` — never the reverse for auth ports.

### Connection ownership (fixed)

- **One SQLite file:** `R1_DATA_DIR/projects.sqlite`
- **Sole factory:** `openSqliteStore({ dbPath }) => { projectRepo, authRepo, close }`
- Internally: open one `better-sqlite3` Database → project migrate → auth migrate → construct both repos sharing that connection → `close()` closes it once
- `AuthRepository` adapter must **not** open or close a connection
- `bootstrapPersistRuntime` calls only `openSqliteStore`
...
DB stores only `sha256(rawCsrf)`. For every CSRF-protected route the server **requires all of**:

1. Cookie `blocksync_csrf` present (missing → **403**)
2. Header `X-CSRF-Token` present (missing → **403**)
3. Header value **equals** Cookie value (mismatch → **403**)
4. `sha256(header) === sessions.csrf_hash` (mismatch → **403**)

Do **not** treat either channel as optional — each absence is independently 403.
...
- CORS (credentials mode):
  - Allow-listed `Origin`: echo exact `Access-Control-Allow-Origin`, `Access-Control-Allow-Credentials: true`, `Vary: Origin`
  - Disallowed / missing Origin: **do not** emit ACAO/ACAC success headers
  - `OPTIONS` preflight for allow-listed Origin must succeed with the same ACAO/ACAC/Vary rules
...
`SessionAuthContext` (in `session-service`) ignores spoof headers; enforces membership on every resolve.
...
- Mutation: missing CSRF Cookie → 403; missing `X-CSRF-Token` → 403; cookie/header mismatch → 403; hash mismatch → 403
- `/v1/auth/google` without Origin → 403; wrong Origin → 403 (login has no CSRF)
- Mutation without Origin → 403 even with valid session+csrf
- CORS: allow-listed OPTIONS preflight returns ACAO (exact origin) + ACAC true + Vary: Origin; disallowed Origin gets no ACAO

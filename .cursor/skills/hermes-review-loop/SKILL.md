...
> **運用ルール 13（Hermes 決裁の自己記録義務）**: 上記判定はチャットに出すだけでなく、**Hermes 自身が台帳へ逐語記録**する。記録先は (a) ヘッダー「現在の状態」(b) 案件レジストリ行 (c) 末尾エントリ の 3 か所。チャット出力のみで「出した」とみなさないこと（PR #204 事案の是正）。記録後に `hermes-merge-preflight.sh` が検出できるか確認する。

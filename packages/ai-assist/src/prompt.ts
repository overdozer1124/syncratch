...
function diagramRules(): string {
  return [
    ...
    "- 図は 3〜6 行まで。長くしないこと。",
    "- 図は必ず【/ず】で閉じること。途中で回答を終わらせないこと。",
    "- 図を書き始めたら、閉じるまで他の話を挟まないこと。",
    "- 図は次のどちらか（または両方）:",
...
/** User message used when the first completion was cut off. */
export function buildContinuationUserPrompt(): string {
  return [
    "いまの回答が途中で止まっています。",
    "続きを、途中からでよいので最後まで書いてください。",
    "【ず】を開いたままなら、必ず【/ず】まで書いて閉じること。",
    "そのあと「じぶんでためす いっぽ」を1つ書いて終わってください。",
    "最初から全部をやり直さなくてよいです。",
  ].join("\n");
}
...
function buildProjectUserParts(params: {
  ...
  const closing = [
    ...
    "かならず短い【ず】…【/ず】を1つ入れ、【/ず】まで書ききってください。",
    "回答を途中で止めないでください。最後に「〜してみよう。」で終えてください。",
    "文字だけの長い説明は禁止です。",
